...
const calcularResultados = (arregloFrase, arregloCita) => {
    ...
    const valorHartman = [6, 9, 10, 11, 13, 5, 17, 16, 12, 4, 1, 18, 2, 14, 8, 15, 3, 7];
    ...
    ...resultado,
    diferenciaAbsolutaMenosDos: Math.max(0, Math.abs(resultado.diferencia) - 2),
    }));
    
 
    const agrupadosPorLetras = {
    I: diferenciaAbsolutaMenosDos.filter((r) => [5, 4, 1, 18, 14, 15].includes(r.hartman)),
    E: diferenciaAbsolutaMenosDos.filter((r) => [6, 11, 13, 17, 2, 8].includes(r.hartman)),
    S: diferenciaAbsolutaMenosDos.filter((r) => [9, 10, 16, 12, 3, 7].includes(r.hartman)),
    };
    
 
    const sumaTotalesDim = {
    "DIM-I": agrupadosPorLetras.I.reduce((acc, r) => acc + Math.abs(r.diferencia), 0),
    "DIM-E": agrupadosPorLetras.E.reduce((acc, r) => acc + Math.abs(r.diferencia), 0),
    "DIM-S": agrupadosPorLetras.S.reduce((acc, r) => acc + Math.abs(r.diferencia), 0),
    };
    
 
    const sumaTotalesInt = {
    "INT-I": agrupadosPorLetras.I.reduce((acc, r) => acc + r.diferenciaAbsolutaMenosDos, 0),
    "INT-E": agrupadosPorLetras.E.reduce((acc, r) => acc + r.diferenciaAbsolutaMenosDos, 0),
    "INT-S": agrupadosPorLetras.S.reduce((acc, r) => acc + r.diferenciaAbsolutaMenosDos, 0),
    };
    
 
    const dif = Object.values(sumaTotalesDim).reduce((acc, dim) => acc + dim, 0);
    
 
    const dimConMayorPuntaje = Object.entries(sumaTotalesDim).sort(([, a], [, b]) => b - a)[0][0];
    const mayorPuntaje = sumaTotalesDim[dimConMayorPuntaje];
    
 
    const intConMayorPuntaje = Object.entries(sumaTotalesInt).sort(([, a], [, b]) => b - a)[0][0];
    ...
    const intGeneral = Object.values(sumaTotalesInt).reduce((acc, dim) => acc + dim, 0);
    ...
    (contador, r) => contador + (String(r.hartman).length !== String(r.num).length),
    0
    );
    ...
    letra,
    {
    sumaPositivas: valores.reduce((acc, r) => acc + (r.diferencia > 0 ? r.diferencia : 0), 0),
    sumaNegativas: valores.reduce((acc, r) => acc + (r.diferencia < 0 ? r.diferencia : 0), 0),
    },
    ])
    );
    
 
    const totalSumaPositivas = Object.values(sumaPorGrupo).reduce((acc, grupo) => acc + grupo.sumaPositivas, 0);
    const totalSumaNegativas = Object.values(sumaPorGrupo).reduce((acc, grupo) => acc + grupo.sumaNegativas, 0);
    const mayorValorNumerico = Math.max(Math.abs(totalSumaPositivas), Math.abs(totalSumaNegativas));
    ...
    // Calcular SQ y VQ
    const sq_1 = resultadosFinalesCita.CitaDif[0] + resultadosFinalesCita.CitaDimGeneral[0] + resultadosFinalesCita.CitaIntGeneral[0] + resultadosFinalesCita.CitaDis[0];
    const sq_2 = sq_1 - resultadosFinalesCita.CitaDif[0];
    const potencialCita = resultadosFinalesCita.CitaIntGeneral[0] / resultadosFinalesCita.CitaDif[0]; // You might want to handle division by zero here
 
    const vq_1 = resultadosFinalesFrase.FraseDif[0] + resultadosFinalesFrase.FraseDimGeneral[0] + resultadosFinalesFrase.FraseIntGeneral[0] + resultadosFinalesFrase.FraseDis[0];
    const vq_2 = vq_1 - resultadosFinalesFrase.FraseDif[0];
    const potencialFrase = resultadosFinalesFrase.FraseIntGeneral[0] / resultadosFinalesFrase.FraseDif[0]; // And here
