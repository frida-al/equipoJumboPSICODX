...
class calificacionesSeriesTerman {
    ...
    async save() {
        const [resultado] = await db.execute(
            'INSERT INTO calificacionesterman (idAspirante, idGrupo, puntosTotales, rango, coeficienteIntelectual) VALUES (?, ?, ?, ?, ?)',
            [
                this.idUsuario, 
                this.idGrupo, 
                this.puntosTotales, 
    ...
    async fetchCalificacionById(idUsuario, idGrupo) {
        const [calificacion] = await db.execute(
            'SELECT * FROM calificacionesterman WHERE idAspirante = ? AND idGrupo = ?',
            [idUsuario, idGrupo]
        );
        return calificacion;
    }

    async updateCalificacionById(nuevoTotal, nuevoRango, nuevoCoeficienteIntelectual, idCalificacionTerman) {
        const [resultado] = await db.execute(
            'UPDATE calificacionesterman SET puntosTotales = ?, rango = ?, coeficienteIntelectual = ? WHERE idCalificacionTerman = ?',
            [nuevoTotal, nuevoRango, nuevoCoeficienteIntelectual, idCalificacionTerman]
        );
        return resultado;
    }
